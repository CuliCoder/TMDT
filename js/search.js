  ...
  function renderSuggestions(products, query) {
    ...
    products.slice(0, 5).forEach((product) => {
      ...
      suggestionItem.addEventListener("click", function () {
        const urls = ["smartphones.html", "accessories.html"];
        const urlDetail = ["smartphone-detail.html", "phukien-detail.html"];
        const currentUrl = window.location.pathname.split("/").pop();
        const isSmartphonePage = urls.includes(currentUrl);
        const isDetailPage = urlDetail.includes(currentUrl);
        const url =
          product.category_id === 1
            ? `${isSmartphonePage ? ".." : "."}${
                isDetailPage ? "/products" : ""
              }/smartphone-detail.html?ProductItemID=${product.id}`
            : `${isSmartphonePage ? ".." : "."}${
                !isDetailPage ? "/products" : ""
              }/phukien-detail.html?ProductItemID=${product.product_id}`;
