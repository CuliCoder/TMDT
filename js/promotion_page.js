...
function formatDateTime(date) {
    return new Date(date).toLocaleString('sv-SE', { timeZone: 'UTC' }).replace('T', ' ');
}
            ...
            var html = data.map(item => {
                return `
                    <div class="row border-bottom py-2">
                        ...
                        <div class="col">${formatDateTime(item.StartDate) || 'trống'}</div>
                        <div class="col">${formatDateTime(item.EndDate) || 'trống'}</div>
                        <div class="col">${formatDateTime(item.CreatedAt) || 'trống'}</div> <!-- Thêm Ngày tạo -->
                        <div class="col">
                            <div class="prom-btn-container"> <!-- Thêm container để chứa các nút -->
                                ...
                                <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#chooseProductsModal" onclick="setPromotionID(${item.PromotionID})">Chọn sản phẩm</button>
                            </div>
                        </div>
                    </div>
                `;
...
document.addEventListener('DOMContentLoaded', function () {
    document.getElementById('promo-text_search').addEventListener('keyup', search);
...
async function submitPromotion() {
    ...
    let discountRate = parseFloat(document.getElementById("discountRate").value.trim());
...
function search() {
    var searchValue = document.getElementById('promo-text_search').value.toLowerCase().trim();

    // Lọc dữ liệu theo tên hoặc mô tả khuyến mãi
    filterData = data.filter(item => {
        var matchText = !searchValue || item.PromotionName.toLowerCase().includes(searchValue)
...
function editPromotion(PromotionID) {
    ...
    let startDateFormatted = new Date(promo.StartDate).toISOString().split('T')[0];
...
async function saveEdit() {
    ...
    const response = await fetch(`http://localhost:3000/api/promotions/${id}`, {
        ...
        body: JSON.stringify({
            ...
            DiscountRate: parseFloat(document.getElementById("editDiscountRate").value.trim()),
    ...
    if (confirm("Bạn có chắc chắn muốn xóa khuyến mãi này không?")) {
        try {
            ...
            if (response.ok) {
                ...
                alert(`Xóa khuyến mãi thất bại: ${errorData.message || 'Không rõ lỗi'}`);
            }
        } catch (error) {
            alert("Có lỗi xảy ra, vui lòng thử lại!");
        }
