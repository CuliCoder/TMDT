...
async function show_list_product() {
  try {
    ...
    for (let i = 0; i < res_product_item.data.data.length; i++) {
      ...
      res_product_item.data.data[i].attributes.forEach((attribute) => {
      ...
      let res_product = await axiosInstance.get(
        `/product/get_product_by_productID/${res_product_item.data.data[i].product_id}`
      );
      document.querySelector(
        ".list_product_import"
      ).innerHTML += `<div class = "box_product_import">
                <img src="http://localhost:3000${
                  res_product_item.data.data[i].product_image
                }" alt="">
                <div class = "box_infor_product_import">
                    <p>ID: ${res_product_item.data.data[i].id}</p>
                    <p>ID product: ${res_product.data.ProductID}</p>
                    <p>${res_product.data.ProductName} ${ram ? ram : ""} ${
        gb ? gb : ""
      } ${color ? color : ""}</p>
...
function event_xacnhan(id) {
  ...
  //     // NgayNhap: xuống dưới be làm ngày nhập
  // }
  let existingItem = orderImport.detailImport.find(
    (item) => item.id_item_product === id
  );
  if (existingItem) {
...
document.querySelector(".form-select").addEventListener("change", function () {
  ...
  if (Number(valueNcc) != 0) orderImport.idNcc = Number(valueNcc);
});
  ...
  for (let i = 0; i < orderImport.detailImport.length; i++) {
    html += `
        <tr>
            ...
            <td>${orderImport.detailImport[i].price.toLocaleString(
              "vi-VN"
            )} VNĐ</td>
            <td><button type="button" class="btn btn-danger" onclick="delete_item(${
              orderImport.detailImport[i].id_item_product
            })">Xóa</button></td>
        </tr>
        `;
  }
...
function delete_item(id) {
  orderImport.detailImport = orderImport.detailImport.filter(
    (item) => item.id_item_product !== id
  );
  event_show_detail_Import();
}
...
// }
document
  .querySelector(".btn_xacnhan")
  .addEventListener("click", async function () {
    orderImport.idNhanVien = "log";
    let Temp = 0;
    for (let i = 0; i < orderImport.detailImport.length; i++) {
      Temp +=
        Number(orderImport.detailImport[i].price) *
        Number(orderImport.detailImport[i].qty);
    }
    ...
    const response = await axiosInstance.post("/product/import_product", {
      orderImport: orderImport.detailImport,
      idsupplier: orderImport.idNcc,
      total_price: Temp,
    });
...
document
  ...
  .addEventListener("input", async function () {
    ...
    for (let item of res_product_item.data.data) {
      let res_product = await axiosInstance.get(
        `/product/get_product_by_productID/${item.product_id}`
      );
      if (
