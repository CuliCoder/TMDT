...
function filter_date(date) {
  let from = document.querySelector("#date-begin").valueAsDate;
  let to = document.querySelector("#date-end").valueAsDate;
  if (from <= new Date(date) && new Date(date) <= to) return true;
  return false;
}
  ...
  for (let i = 0; i < users.length; i++) {
    for (let j = 0; j < users[i].order.length; j++) {
      if (
        ...
        filter_date(users[i].order[j].date_purchase)
      ) {
        //chỉ tính những đơn hàng đã được xác nhận
        for (let v = 0; v < users[i].order[j].products.length; v++) {
          if (users[i].order[j].products[v].brand == brand) {
...
document.querySelector(".search-box button").addEventListener("click", () => {
  ...
  let total = [0, 0];
  for (let i = 0; i < array_brand.length; i++) {
    let Filter = filter(array_brand[i]); //lấy số lượng và tổng tiền từng hãng
    total[0] += Filter[0]; //tổng sl tất cả hãng
    total[1] += Filter[1]; //tổng tiền tất cả hãng
    document.querySelector(
      ".list-brands .brands"
    ).innerHTML += `<li class="brand">
    <ul>
